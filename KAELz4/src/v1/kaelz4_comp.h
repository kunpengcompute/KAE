...
struct kaelz4_priv_save_info {
    void *
        prev_last_lit_ptr; // 用户输入数据>64K需要分块、返回BLOCK格式、现有保序返回切块压缩结果的约束下，记录前一个分块的last literal信息
    size_t prev_last_lit_len;
    unsigned int
        prev_last_lit_buf_index; // 用户输入数据>64K需要分块、返回BLOCK格式、现有保序返回切块压缩结果的约束下，记录前一个分块的last literal信息
...
typedef int (*kaelz4_post_process_handle_t)(struct kaelz4_async_req *req, const struct wd_buf_list *source, void *dest,
...
static inline void wd_ip_add_len(
    const struct wd_buf_list *src, unsigned int *cur_idx, const BYTE **ip, size_t *remain, size_t len)
{
...
static inline void wd_copy_from_buffers(
    const struct wd_buf_list *src, unsigned int *cur_idx, const BYTE **ip, size_t *remain, BYTE *dst, size_t len)
{
    while (len > 0 && *cur_idx < src->buf_num) {
...
static inline void kaelz4_ip_add_len(
    const struct kaelz4_buffer_list *src, unsigned int *cur_idx, const BYTE **ip, size_t *remain, size_t len)
{
    while (len > 0 && *cur_idx < src->buf_num) {
        if (*remain > len) {
            *ip += len;
...
static inline void kaelz4_copy_from_buffers(
    const struct kaelz4_buffer_list *src, unsigned int *cur_idx, const BYTE **ip, size_t *remain, BYTE *dst, size_t len)
{
    while (len > 0 && *cur_idx < src->buf_num) {
        size_t can_copy = *remain < len ? *remain : len;
        LZ4_memcpy(dst, *ip, can_copy);

...
inline KaeZstdConfig* kaezstd_get_config(LZ4_CCtx* zc)
{
...
inline void kaezstd_set_config(LZ4_CCtx* zc, KaeZstdConfig* config)
{
    if (zc != NULL) {
...
int kaezstd_init_v2(LZ4_CCtx* zc)
{
    int ret;
...
void kaezstd_release_v2(LZ4_CCtx* zc)
{
    KaeZstdConfig *config = NULL;
    if (zc == NULL) {
        return;

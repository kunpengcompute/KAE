...
static void compress_async_polling(struct compress_param *param)
{
    ...
    ctx->algorithm->prepare_outbuf(ctx, out_buf, param);
...
static int do_real_compression(struct compress_ctx *ctx, struct compress_param *param)
{
    if (ctx->compress_or_decompress) { // 压缩流程。
        if (ctx->algorithm->async_compress) {
            if(ctx->sess_count > 1) {
                int idx = get_session_index(ctx);
                return ctx->algorithm->async_compress(&ctx->sess_array[idx],  param);
            }
            return ctx->algorithm->async_compress(&ctx->sess,  param);
        } else {
            return ctx->algorithm->compress(param);
        }
    } else { // 解压逻辑
        if (ctx->algorithm->async_decompress) {
            if(ctx->sess_count > 1) {
                int idx = get_session_index(ctx);
                return ctx->algorithm->async_decompress(&ctx->sess_array[idx], param);
            }
            return ctx->algorithm->async_decompress(&ctx->sess, param);
        } else {
            return ctx->algorithm->decompress(param);
        }
    }
    return 0;
}
...
static void check_and_reset_src_buf_num(struct compress_ctx *ctx) {
    ...
    if (((size_t)g_file_chunk_size * 1024) <= HPAGE_SIZE && has_async_test) {
        ...
        // 在QAT中小块分片性能测试时，使用单个节点性能最优。
        if(g_file_chunk_size < 128) {
            ctx->src_buf_num = 1;
                      ...
                      const char* in_filename, const char* out_filename,int multi)
{
    ...
    if (g_log_level) {
        ...
        if (cahr_print_table_data != NULL) {
            ...
            printf( "console.log('kae-threads:%d, multi:%d,file:%s. chunk %d kb. inflightNum: %d, alg:%s ');",
                kae_task_num, multi, in_filename, g_file_chunk_size,  ctx->inflight_num, ctx->algorithm->name);
    ...
    struct compress_ctx *ctx, void *inbuf, uLong src_len, void *outbuf, uLong output_sz, unsigned long *out_offset)
{
    ...
    while (remaining > 0) {
        ...
        param->src_buf = inbuf;
        param->src_buf_offset = start_buf - inbuf;
        param->src_len = chunk_len_this_loop;
        param->dst_buf = dst_start;
        param->dst_len = output_sz_tmp;
        param->ctx = ctx;
        param->sn = ctx->sn;
        param->loop_index = ctx->loop_index;
        if (ctx->algorithm->prepare_param)
            ctx->algorithm->prepare_param(ctx, param);

        ret = do_real_compression(ctx, param);
        if (ret != 0) {
...
{
    size_t tuple_buf_len = ((size_t)g_file_chunk_size * 1024) * (src_len / ((size_t)g_file_chunk_size * 1024) + 1) * 8;
    ...
    for (j = 0; j < ctx->loop_times; j++) {
        ...
        if (g_file_chunk_size != 0) { // 分片逻辑
            for (int k = 0; k < fragment_count; k++) {
                ...
                param->done = 2;
                ctx->loop_index = j; // 原来没有这一句，为了适配 param->loop_index = j
                // param->loop_index = j;
                ...
                param->src_buf = inbuf;
                param->src_buf_offset = this_offset;
                param->src_len = this_src_len;
                param->dst_buf = dst_start;
                param->dst_len = output_size_chunk;
                param->ctx = ctx;
                param->sn = ctx->sn;
                param->loop_index = ctx->loop_index;
                if (ctx->algorithm->prepare_param)
...
int main(int argc, char **argv)
{
    ...
    const char *optstring = "dm:l:n:w:f:o:v:A:hg:s:c:i:t:r:P:p:e:";
    int ret = 0;
    int o = 0;
    int multi = 1;
    int level = 6;

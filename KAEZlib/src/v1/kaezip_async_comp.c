...
static void kaezip_compress_async_callback(struct kaezip_compress_ctx *compress_ctx, int status)
{
    ...
    if (result->ibuf_crc != NULL && status == KAE_ZLIB_SUCC && compress_ctx->ibuf_checksum_flag != 1) {
        for (int i = 0; i < compress_ctx->src->buf_num; i++) {
            *result->ibuf_crc =
                KAEZIPCRC32(*result->ibuf_crc, compress_ctx->src->buf[i].data, compress_ctx->src->buf[i].buf_len);
        }
    }
...
static int kaezip_compress_async_impl(kaezip_ctx_t *kz_ctx, const struct wd_buf_list *src, struct wd_buf_list *dst,
    size_t srcSize, size_t dst_len, void *usr_data)
{
...
int kaezip_async_compress_polling(struct kaezip_async_ctrl *ctrl, int budget)
{
    ...
    while (req && cnt < budget) {
        ...
        if (likely(compress_ctx->status == KAE_ZLIB_SUCC)) {
            ret = compress_ctx->kaezip_post_process_handle(
                req, &req->src, compress_ctx->dst->buf[0].data + compress_ctx->dst_len, &compress_ctx->save_info);
            if (ret < 0) {
...
static int kaezip_send_async_compress(
    struct kaezip_async_ctrl *ctrl, struct kaezip_async_req *req, int comp_optype, int comp_algtype)
{
    // 1.kae上下文初始化函数调用
...
static void kaezip_fill_hw_req_dst_buf_list(
    struct kaezip_async_ctrl *ctrl, struct kaezip_async_req *req, const struct kaezip_buffer_list *dst)
{
...
static void kaezip_fill_hw_req_src_buf_list(
    struct kaezip_async_ctrl *ctrl, struct kaezip_async_req *req, const struct kaezip_buffer_list *src)
{
    unsigned int index = 0;
...
static uint32_t extract_checksum(struct kaezip_async_req *req, unsigned int output_len)
{
    ...
    uint32_t checksum =
        ((uint32_t)result[3] << 24) | ((uint32_t)result[2] << 16) | ((uint32_t)result[1] << 8) | (uint32_t)result[0];
    return checksum;
}
#endif

static int kaezip_async_block_padding(struct kaezip_async_req *req, const struct wd_buf_list *source, void *dst_tmp,
    struct kaezip_priv_save_info *save_info)
{
    kaezip_ctx_t *kz_ctx = req->kz_ctx;
    struct wcrypto_comp_op_data *op_data = &kz_ctx->op_data;

    unsigned int hw_output_len = op_data->produced; // actual data length generated by the hardware
    unsigned int final_output_len = hw_output_len;  // final returned length (includes the ZLIB header).
    unsigned int copy_offset = 0;                   // skip 2 bytes header for zlib format
    if (req->kz_ctx[0].comp_type == WCRYPTO_DEFLATE && req->kz_ctx[0].comp_alg_type == WCRYPTO_GZIP) {
        ...
        hw_output_len -= 8;
        final_output_len = hw_output_len;
    ...
    if (!kz_ctx->q_node->is_sgl) {
        ...
        for (unsigned int i = 0; i < dst->buf_num; i++) {
            ...
            capacity += (i == 0 && copy_offset > 0) ? (dst_buf->buf_len - copy_offset) : dst_buf->buf_len;
            // all data has been copied
            if (copied >= hw_output_len) {
                break;
            }
            // get the starting address and copyable length of the current buffer
            unsigned char *dst_ptr = (unsigned char *)dst_buf->data;
            unsigned int available = dst_buf->buf_len;
            if (i == 0 && copy_offset > 0) {
                dst_ptr += copy_offset;
                available -= copy_offset;
            }
            // get the length of data that has not been copied
            unsigned int remaining = hw_output_len - copied;
            // get the length of data that can be copied
            unsigned int to_copy = (remaining < available) ? remaining : available;
            memcpy(dst_ptr, op_data->out + copied, to_copy);
            copied += to_copy;
        }
        if (hw_output_len > capacity) {
            US_ERR("The length of produced data exceeds the capacity of the dst buffer list.");
            return KAE_ZLIB_DST_BUF_OVERFLOW;
        }
    }

    return final_output_len;
}
...
int kaezip_compress_async(struct kaezip_async_ctrl *ctrl, const struct kaezip_buffer_list *src,
    struct kaezip_buffer_list *dst, kaezip_async_callback callback, struct kaezip_result *result,
...
int scan_hisi_zip_devices(struct zip_dev *g_devices, unsigned int *g_dev_count)
{
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        snprintf(
            temp_devs[count].dev_root, sizeof(temp_devs[count].dev_root), "%s/%s", UACCE_CLASS_PATH, entry->d_name);

...
const unsigned char sm4_cbc_iv[] = {
...
int kae_sm4_cbc_encrypt(ENGINE *engine, const unsigned char *plantext, int plen, unsigned char *outbuf, int * outlen)
{
     ...
     */
    // if (!EVP_EncryptInit_ex2(ctx, cipher, sm4_cbc_key, sm4_cbc_iv, /* params */ NULL))
    //     goto err;
    if (!EVP_EncryptInit_ex(ctx, cipher, engine, sm4_cbc_key, sm4_cbc_iv))
        goto err;
...
int kae_sm4_cbc_decrypt(ENGINE *engine, const unsigned char *ciphertext, int clen, unsigned char *outbuf, int * outlen)
{
     ...
     * We are not setting any custom params so let params be just NULL.
     */
    // if (!EVP_DecryptInit_ex2(ctx, cipher, sm4_cbc_key, sm4_cbc_iv, /* params */ NULL))
    //     goto err;
    if (!EVP_DecryptInit_ex(ctx, cipher, engine, sm4_cbc_key, sm4_cbc_iv))
        goto err;

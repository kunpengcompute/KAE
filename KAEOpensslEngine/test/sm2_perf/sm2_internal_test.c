...
////////////////////////////////////////////////////////////////////////////////////////////////////////
// 生成随机数
void generate_random(unsigned char *buf, int len) {
...
int do_multi_perf(int multi, ulong stream_len, int loop_times)
{
    int i, j;
    int ret = 0;
    pid_t pid_child = 0;
    ...
    if (pid_child == 0) {
        ...
        for (j = 0; j < loop_times; j++) {
            if (g_testcase == 1) {
                sm2_crypt_test();
            } else if (g_testcase == 2) {
                sm2_sig_test();
            } else if (g_testcase == 0) {
                sm2_crypt_test();
                sm2_sig_test();
            } else {
                printf("[error]test case is wrong.\n");
            }
        }
    } else if (pid_child > 0) {
        ret = -1;
    ...
    if (pid_child > 0 || multi == 0) {
        ...
        if (time1 == 0) {
            ...
            speed1 = (loop_times * multi * 1000000) / time1;
            // 进行后续操作
        }
        printf("openssl sm2 perf result:\n");
        printf(" loottime is %d,  time used: %lu us, speed = %.6f pic/s\n", loop_times, time1, speed1);
    }
...
void usage(void)
{
    ...
    printf("  -n: loop times\n");
    printf("  -t: test type: 1 for sm2 crypto, 2 for sm2 sign, 0 for all\n");
    printf("  example: ./sm2_test_perf -m 1 -t0 -n 1000\n");
}

int main(int argc, char **argv)
{
    int o = 0;
    const char *optstring = "dm:l:n:t:w:f:o:h";
    int multi = 1;
    ...
    printf("openssl sm2: multi process %d, stream length: %lu(KB), loop times: %d, g_testcase is :%d.\n", 
        multi, stream_len, loop_times, g_testcase);
    ...
    return do_multi_perf(multi, stream_len, loop_times);

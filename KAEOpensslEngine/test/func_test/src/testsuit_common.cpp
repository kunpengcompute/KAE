...
//RSA 公钥加密
int rsa_public_encrypt(RSA *key, unsigned char *encData, unsigned char *srcStr, unsigned int padding_mode)
{
    int ret;

    ret = RSA_public_encrypt(strlen((const char *)srcStr), srcStr, encData, key, padding_mode);
    if (ret < 0) {
...
// 私钥解密
int rsa_private_decrypt(RSA *key, unsigned char *decData, unsigned char *encData, size_t enclen, unsigned int padding_mode)
{
    int declen;

    declen = RSA_private_decrypt(enclen, encData, decData, key, padding_mode);
    if (declen < 0) {
        unsigned long ulErr = ERR_get_error();  
        char szErrMsg[1024] = { 0 };
        ERR_error_string(ulErr, szErrMsg);  
...
// 私钥加密
int rsa_private_encrypt(RSA *key, unsigned char *encData, unsigned char *srcStr, unsigned int padding_mode)
{
    int ret;

    if (padding_mode != RSA_PKCS1_PADDING && padding_mode != RSA_NO_PADDING) {
        //私钥加密公钥解密只能使用RSA_PKCS1_PADDING
        return -1;
    }
    
    ret = RSA_private_encrypt(strlen((const char *)srcStr), srcStr, encData, key, padding_mode);
    if (ret < 0) {
        unsigned long ulErr = ERR_get_error();  
        char szErrMsg[1024] = { 0 };
        ERR_error_string(ulErr, szErrMsg);  
...
// 公钥解密
int rsa_public_decrypt(RSA *key, unsigned char *decData, unsigned char *encData, size_t enclen, unsigned int padding_mode)
{
    int declen;
    ...
    declen = RSA_public_decrypt(enclen, encData, decData, key, padding_mode);
    if (declen < 0) {
        unsigned long ulErr = ERR_get_error();  
        char szErrMsg[1024] = { 0 };
        ERR_error_string(ulErr, szErrMsg);  
...
{   
    ...
    enclen = rsa_public_encrypt(rsa, encData, (unsigned char *)srcStr, padding);
    if (enclen < 0) {
        printf("Encryption failed.\n");
        return FALSE;
    }
    declen = rsa_private_decrypt(rsa, decData, encData, enclen, padding);
    if(declen < 0) {
    ...
    if (padding == RSA_PKCS1_PADDING || padding ==RSA_NO_PADDING) {
        //私钥加密公钥解密只能使用RSA_PKCS1_PADDING
        siglen = rsa_private_encrypt(rsa, signData, (unsigned char *)srcStr, padding);
        if(siglen < 0) {
            printf("Failed to sign the signature.\n");
            return FALSE;
        }
        verlen = rsa_public_decrypt(rsa, verData, signData, siglen, padding);
        if(verlen < 0) {

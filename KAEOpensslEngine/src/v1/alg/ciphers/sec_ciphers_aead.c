...
void wd_aeads_put_engine_ctx(aead_engine_ctx_t *e_aead_ctx)
{
    ...
    // if (e_aead_ctx != NULL) {
	// 	wd_aeads_free_engine_ctx(e_aead_ctx);
	// 	e_aead_ctx = NULL;
	// }
...
static int sec_aead_engine_cleanup(aead_priv_ctx_t *priv_ctx)
{
	...
	// if (priv_ctx->data_buf != NULL) {
	// 	kae_free(priv_ctx->data_buf);
	// }

	// if (priv_ctx->out_data_buf != NULL) {
	// 	kae_free(priv_ctx->out_data_buf);
	// }
			       ...
			       const unsigned char *iv, int encrypt)
{
    ...
    // opdata的基本参数设置 ==>放到计算流程去
	priv_ctx->data_buf = priv_ctx->e_aead_ctx->op_data.in; //databuf 事情20M
	priv_ctx->out_data_buf = priv_ctx->e_aead_ctx->op_data.out; //databuf 事情20M
...
{
	/* Asynchronous jobs use the block mode. 异步模式之后调试再看*/

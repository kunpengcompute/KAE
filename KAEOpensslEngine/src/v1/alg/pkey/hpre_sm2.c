...
err:
	// (void)wd_ciphers_free_engine_ctx(e_sm2_ctx); todo
...
static int wd_sm2_init_engine_ctx(hpre_sm2_engine_ctx_t *e_sm2_ctx)
{
    ...
    // e_sm2_ctx->setup.cv.cfg.id = WCRYPTO_BRAINPOOLP320R1;// 疑问
...
static int hpre_sm2_init_priv_ctx(hpre_sm2_priv_ctx_t *priv_ctx, EVP_PKEY_CTX *ctx) {
    HPRE_SM2_RETURN_FAIL_IF(priv_ctx == NULL, "priv_ctx is NULL", OPENSSL_FAIL);
    HPRE_SM2_RETURN_FAIL_IF(ctx == NULL, "ctx is NULL", OPENSSL_FAIL);
    if (priv_ctx->e_hpre_sm2_ctx == NULL) {
		priv_ctx->e_hpre_sm2_ctx = wd_sm2_get_engine_ctx(priv_ctx);
		if (priv_ctx->e_hpre_sm2_ctx == NULL) {
...
static int  hpre_sm2_init(EVP_PKEY_CTX *ctx)
{
    ...
    //初始化一些需求malloc的空间
    // if (hpre_sm2_init_priv_ctx(sm2ctx, ctx) != OPENSSL_SUCCESS) {
		       ...
		       const unsigned char *in, size_t inlen)
{
	...
	if (!ret) {
		ret = OPENSSL_FAIL;
		fprintf(stderr, "failed to hpre_sm2_crypto, ret = %d\n", ret);
				...
				struct wd_dtb *c3)
{
	...
	ecc_out = wcrypto_new_sm2_enc_out(sess, c2->dsize);
	if (!ecc_out) {
		fprintf(stderr, "failed to new dec out\n");
		return OPENSSL_FAIL;
	}

	ecc_in = wcrypto_new_sm2_enc_in(sess, c2, c3);
	if (!ecc_in) {
		       ...
		       const unsigned char *in, size_t inlen)
{
	...
	memset(&opdata, 0, sizeof(opdata));
	ret = hpre_sm2_decrypt_init_iot(smctx->e_hpre_sm2_ctx->wd_ctx, &opdata, &c2, &c3);
	if (!ret)
		goto free_c1;

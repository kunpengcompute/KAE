...
static struct ecc_test_ctx *sm2_create_hw_sign_test_ctx(struct ecc_test_ctx_setup setup, u32 op_type)
{
	...
	if (setup.key_from) { // set k, id param and support plaintext or dgst
		...
		ecc_in = wcrypto_new_sm2_sign_in(ctx, &e, &k, NULL, is_dgst);
		if (!ecc_in) {
...
// // print_data(e.data, e.dsize, "edata");
...
int ecc_init_test_ctx_setup(struct ecc_test_ctx_setup *setup, __u32 op_type)
{
	...
	if (setup->nid == 714 || key_bits == 256) { //NID_secp256k1
		...
		if (op_type == SM2_SIGN || op_type == SM2_VERF ||
			...
			setup->kinv = sm2_k;
			setup->kinv_size = sizeof(sm2_k);//BITS_TO_BYTES(set_randparam_len);
...
int main(int argc, char *argv[])
{
	...
	if (!strcmp(argv[1], "-system-qt")) {
		...
		is_system_test = 1;
	} else if (!strcmp(argv[1], "-system-sign-sm2")) {//ok ./test_wd_hpre -system-sign-sm2 -t 1 -c 0x3 -performance -nocheck 256 1
		HPRE_TST_PRT("sm2 sign sync\n");
		alg_op_type = SM2_SIGN;
		is_system_test = 1;
	} else if (!strcmp(argv[1], "-system-verf-sm2")) { //no---> ok  ./test_wd_hpre -system-verf-sm2 -t 1 -c 0x3 -performance -nocheck  256 1
		HPRE_TST_PRT("sm2 verf sync\n");
		alg_op_type = SM2_VERF;
		is_system_test = 1;
	} else if (!strcmp(argv[1], "-system-asign-sm2")) {//ok  ./test_wd_hpre -system-asign-sm2 -t 1 -c 0x3 -performance -nocheck 256 1
		HPRE_TST_PRT("sm2 sign async\n");
		alg_op_type = SM2_ASYNC_SIGN;
		is_system_test = 1;
	} else if (!strcmp(argv[1], "-system-averf-sm2")) {//no //ok ./test_wd_hpre -system-averf-sm2 -t 1 -c 0x3 -performance -nocheck 256 1
		HPRE_TST_PRT("sm2 verf async\n");
		alg_op_type = SM2_ASYNC_VERF;
		is_system_test = 1;
	} else if (!strcmp(argv[1], "-system-enc-sm2")) {//ok ./test_wd_hpre -system-enc-sm2 -t 1 -c 0x3 -performance -nocheck 256 1
		HPRE_TST_PRT("sm2 enc sync\n");
		alg_op_type = SM2_ENC;
		is_system_test = 1;
	} else if (!strcmp(argv[1], "-system-dec-sm2")) {  //nooooo-->ok  ./test_wd_hpre -system-dec-sm2 -t 1 -c 0x3 -performance -nocheck 256 1
		HPRE_TST_PRT("sm2 dec sync\n");
		alg_op_type = SM2_DEC;
		is_system_test = 1;
	} else if (!strcmp(argv[1], "-system-aenc-sm2")) {//ok  ./test_wd_hpre -system-aenc-sm2 -t 1 -c 0x3 -performance -nocheck 256 1
		HPRE_TST_PRT("sm2 enc async\n");
		alg_op_type = SM2_ASYNC_ENC;
		is_system_test = 1;
	} else if (!strcmp(argv[1], "-system-adec-sm2")) { //nooooo-->ok  ./test_wd_hpre -system-adec-sm2 -t 1 -c 0x3 -performance -nocheck  256 1
		HPRE_TST_PRT("sm2 dec async\n");
		alg_op_type = SM2_ASYNC_DEC;
		is_system_test = 1;
	} else if (!strcmp(argv[1], "-system-kg-sm2")) {//ok  ./test_wd_hpre -system-kg-sm2 -t 1 -c 0x3 -performance -nocheck 256 1
		HPRE_TST_PRT("sm2 kg sync\n");
		alg_op_type = SM2_KG;
		is_system_test = 1;
	} else if (!strcmp(argv[1], "-system-akg-sm2")) {//ok  ./test_wd_hpre -system-akg-sm2 -t 1 -c 0x3 -performance -nocheck 256 1
